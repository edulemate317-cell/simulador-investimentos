...
# ==========================================
# BARRA LATERAL (CONTROLES COM TOOLTIPS)
# ==========================================
st.sidebar.header("💰 Configurações Gerais")

# Adicionamos o parâmetro 'help' aqui!
...
aporte = st.sidebar.number_input("Aporte Mensal (R$)", value=500.0, help="Quanto você vai depositar todos os meses. A constância é o verdadeiro motor dos juros compostos!")
meses = st.sidebar.slider("Prazo (Meses)", 1, 120, 24, help="Tempo total do investimento. Dica: Prazos acima de 24 meses (720 dias) pagam a menor taxa de Imposto de Renda possível (15%).")

def menu_ativo(n):
    st.sidebar.markdown(f"### Ativo {n}")
    tipo = st.sidebar.selectbox(
        f"Tipo", ["CDB", "LCI/LCA", "Poupança"], key=f"t{n}", 
        help="CDBs sofrem desconto de Imposto de Renda. LCI, LCA e Poupança são isentos de impostos."
    )
    ...
    if tipo == "Poupança":
        ...
        perc = st.sidebar.number_input(
            "% do CDI", value=100.0, key=f"p{n}", 
            help="Quanto o banco te paga em relação à taxa básica. Ex: Um CDB de 110% do CDI rende mais do que um de 100%."
        )
        taxa = cdi * (perc/100)
        isento = (tipo == "LCI/LCA")
...
with aba_comparador:
    if btn_simular:
        ...
        c1.metric("Dinheiro Investido", f"R$ {total:,.2f}", help="A soma do seu valor inicial com todos os aportes mensais realizados.")
        c2.metric(n1, f"R$ {liq1:,.2f}", f"Ganho Real: R$ {df1['Poder de Compra (Real)'].iloc[-1] - total:,.2f}", delta_color="normal", help="O Ganho Real já desconta a inflação do período. Mostra o quanto você realmente enriqueceu em poder de compra.")
        c3.metric(n2, f"R$ {liq2:,.2f}", f"Ganho Real: R$ {df2['Poder de Compra (Real)'].iloc[-1] - total:,.2f}", delta_color="normal", help="O Ganho Real já desconta a inflação do período.")
...
with aba_carteira:
    ...
    patrimonio_total = st.number_input("Patrimônio Total Disponível (R$)", value=50000.0, step=1000.0, help="O valor total que você tem hoje para dividir entre os diferentes investimentos.")
    
    col_p1, col_p2, col_p3 = st.columns(3)
    perc_cdb = col_p1.slider("Renda Fixa (CDB)", 0, 100, 50, help="Bons para fazer o dinheiro crescer a médio/longo prazo.")
    perc_lci = col_p2.slider("Isentos (LCI/LCA)", 0, 100, 30, help="Excelentes para prazos curtos ou médios por causa da isenção de IR.")
    perc_caixa = col_p3.slider("Reserva de Emergência", 0, 100, 20, help="Dinheiro que precisa estar sempre à mão (liquidez diária).")
